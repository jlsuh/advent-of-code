...
int** possible_dumbos(int rows, int columns, int currX, int currY, int *size) {
    int** moves = NULL;
    int possibleDumbos[POSSIBLE_DUMBOS_SIZE][COMPONENTS_SIZE] = {
        {currX - 1, currY},
        {currX - 1, currY + 1},
        {currX, currY + 1},
        {currX + 1, currY + 1},
    ...
    for(int i = 0; i < POSSIBLE_DUMBOS_SIZE; i++) {
        if(possibleDumbos[i][0] >= 0 && possibleDumbos[i][0] < columns && possibleDumbos[i][1] >= 0 && possibleDumbos[i][1] < rows) {
            moves = realloc(moves, ++(*size) * sizeof(int*));
            moves[*size - 1] = calloc(COMPONENTS_SIZE, sizeof(int));
            moves[*size - 1][0] = possibleDumbos[i][0];
            moves[*size - 1][1] = possibleDumbos[i][1];
        }
    }
    return moves;
}
...
void make_surrounding_dumbos_flash(t_dumbo** dumbos, int rows, int columns, int currX, int currY) {
    int size = 0;
    int** possibleDumbos = possible_dumbos(rows, columns, currX, currY, &size);
    for(int k = 0; k < size; k++) {
        increase_dumbo_energy(&dumbos[possibleDumbos[k][0]][possibleDumbos[k][1]]);
    }
    matrix_destroy((void**) possibleDumbos, size);
}

void make_dumbos_flash(t_dumbo** dumbos, int rows, int columns) {
...
bool all_dumbos_are_flashing(t_dumbo** dumbos, int rows, int columns) {
    for(int i = 0; i < rows; i++) {
        for(int j = 0; j < columns; j++) {
            if(dumbos[i][j].energyLevel != 0) {
                return false;
...
int simulate_steps(t_dumbo** dumbos, int rows, int columns) {
    int currStep = 0;
    ...
    do {
        ...
        make_dumbos_flash(dumbos, rows, columns);
        printf("After step %d:\n", currStep);
        dumbos_print(dumbos, rows, columns);
    } while(!all_dumbos_are_flashing(dumbos, rows, columns));
    return currStep;
}
...
int solution(FILE* input) {
    ...
    int numberOfSteps = simulate_steps(dumbos, rows, columns);
